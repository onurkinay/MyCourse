...
namespace MyCourse.Catalog.API.Features.Courses.GetById;
...
public class GetCourseByIdQueryHandler(AppDbContext context, IMapper mapper) : IRequestHandler<GetCourseByIdQuery, ServiceResult<CourseDto> >
{
    ...
    {
        var hasCourse = await context.Courses.FirstOrDefaultAsync(x => x.Id == request.Id);
    ...
    public static RouteGroupBuilder GetByIdCourseGroupItemEndpoint(this RouteGroupBuilder group)
    {
        group.MapGet("/{id}",
            async (IMediator mediator,Guid id) => (await mediator.Send(new GetCourseByIdQuery(id)))
                .ToGenericResult()).WithName("GetCourseById");
        return group;
    }
}